...
const BOOK_NUMBER_BY_NAME = new Map(BOOKS.map((name, idx) => [name, idx + 1]))
export const DEFAULT_BIBLE_TRANSLATION = 'esv'
...
export function makeVerseId({ book, refKey, translation = DEFAULT_BIBLE_TRANSLATION }){
  const tid = normalizeTranslationId(translation)
  return `v:${tid}|${book} ${refKey}`.trim()
}

export function parseVerseReference(raw, options = {}){
  ...
  const translation = normalizeTranslationId(options.translation)
  const id = makeVerseId({ book: resolved.book, refKey, translation })
  return {
    translation,
    book: resolved.book,
...
function resolveBook(rawBook){
  ...
  if (alias) return { book: alias, bookNumber: bookNumberFromName(alias) }

  const exact = BOOK_KEYS.find((b) => b.key === key)
  if (exact) return { book: exact.name, bookNumber: bookNumberFromName(exact.name) }

  const matches = findBookMatches(key)
  if (matches.length === 1) return { book: matches[0], bookNumber: bookNumberFromName(matches[0]) }
...
export function normalizeTranslationId(raw){
  const cleaned = String(raw || '')
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9_-]/g, '')
  return cleaned || DEFAULT_BIBLE_TRANSLATION
}

export function bookNumberToName(bookNumber){
  const n = Number(bookNumber)
  if (!Number.isFinite(n)) return ''
  return BOOKS[n - 1] || ''
}

function bookNumberFromName(book){
  return BOOK_NUMBER_BY_NAME.get(book) || 0
}
