...
## Public reflections moderation (backend — RETIRED)
- **The public "Shared Reflections" feature is gone.** App Review rejected 1.0.0 (11) under Guideline 1.2 (anonymous user-generated content); PR 469 deleted every client surface and narrowed `ReflectionVisibility` to `'private'`, and migration `20260805000000_retire_public_reflections_age_gate.sql` retired the backend — `public_reflections` flag **off**, `public_feed_read` policy **dropped**. Reflections are now a private per-user journal only. Do not build against anything below without reopening the Guideline 1.2 question first; the rest of this section is kept because the code still exists and is still deployed.
- **Public reflections could only enter the DB after passing moderation.** The reflections `own_insert` RLS policy is private-only, so a `visibility='public'` row was written **exclusively** by the service-role Pages Function `functions/api/reflections/submit.js` — there was never a client-facing public-insert policy, which guaranteed moderation always ran. That endpoint (and `report.js`) is **still deployed** — Cloudflare Pages routes by file, so it responds whether or not a client calls it — but with the kill switch off it now refuses at its own flag check (`submit.js:68`) instead of inserting.
