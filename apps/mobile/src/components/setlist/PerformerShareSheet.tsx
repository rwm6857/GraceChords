  ...
  return (
    <FormSheetShell title={tx('title')} onAction={onClose}>
      <View style={{ padding: t.spacing.lg, paddingBottom: t.spacing.lg + insets.bottom, gap: t.spacing.md }}>
        ...
        {scope === 'song' ? (
          <>
            {primaryButton(tx('exportAsPdf'), 'square.and.arrow.up', 'song-pdf', () => handlers.onExportSong('pdf'))}
            {secondaryRow(tx('exportAsJpg'), 'photo', 'song-jpg', () => handlers.onExportSong('jpg'))}
